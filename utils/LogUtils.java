...
public class LogUtils {
    ...
    private static void log(String tag, String msg, Throwable tr, char level) {
        if (LOG_SWITCH) {
            ...
            if (LOG_TO_FILE)
                logtoFile(String.valueOf(level), tag, msg + tr == null ? "" : "\n" + Log.getStackTraceString(tr));
        }
    }
     ...
     * @return
     **/
    private synchronized static void logtoFile(String mylogtype, String tag, String text) {
        Date currentTime = new Date();
        String date = FILE_SUFFIX.format(currentTime);
        String dateLogContent = LOG_FORMAT.format(currentTime) + ":" + mylogtype + ":" + tag + ":" + text; // 日志输出格式
        //创建日志输出文件目录
